...
-(void)changeTheme:(UIButton*)button
{
    ...
    if (button.tag==3) {
        ...
        sql = [database doQuery:[NSString stringWithFormat:@"update preferencias set theme=%d",button.tag]];
        [self recreateClock];
    }
...
-(void)showMasterControls:(float)alpha
{
    ...
    if (alpha==1) {
        ...
        if(configView.frame.size.height>1000) configView.frame = CGRectMake(0, -130, h, 130);
...
- (void)viewDidLoad
{
    ...
    if (configTheme==3) {
        ...
        NSLog(@"veio config view did load");
